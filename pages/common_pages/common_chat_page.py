...
class CommonChatPage(BasePage):
    ...
    def _navigate_to_home(self):
        ...
        try:
            ...
            log.debug("Optional step in _navigate_to_home() did not apply: %s", _ignored)
    ...
    def click_chat_icon(self):
        ...
        self.click_first_visible([HomeLocators.ACCOUNTS_MENU_TRIGGER], "accounts menu trigger", timeout=8000)
        self.pause(500)

        self.click_required([
            HomeLocators.DROPDOWN_MESSAGES_ITEM,
            "//*[contains(normalize-space(),'Messages & Discussions')]",
        ], "Messages & Discussions menu item")

    def click_send_message_button(self):
        self.click_required([CommonChatLocators.SEND_MESSAGE_BUTTON], "Send Message button")
    ...
    def new_message_flow_available(self, timeout=4000):
        ...
        ], "Error popup appeared but the 'Go to Homepage' button", timeout=4000)
        log.info("'Oops! Something went wrong.' popup validated with 'Go to Homepage' button.")
        # Recover by clicking Go to Homepage so we land on a clean screen.
        self.click_required([
            "//button[normalize-space()='Go to Homepage']",
        ], "'Go to Homepage' button", timeout=4000)
        self.pause(1500)
        return False

    def click_first_contact(self):
        self.click_required([CommonChatLocators.FIRST_NEW_MESSAGE], "First contact in list", timeout=15000)
        ...
        composer_or_send = self.first_visible_in_any_frame([
        ...
        if not composer_or_send:
            self.click_first_visible([CommonChatLocators.FIRST_NEW_MESSAGE], "first new message", timeout=10000)
    ...
    def send_message(self, message_text=None):
        ...
        composer = self.first_visible_in_any_frame(composer_selectors, timeout_per_try=2500)
        if not composer:
        ...
        if not clicked:
            send_btn = self.first_visible_in_any_frame([
    ...
    def validate_latest_message_sent(self, expected_text=None):
        ...
        try:
            ...
            log.debug("Optional step in validate_latest_message_sent() did not apply: %s", _ignored)
        self.pause(1000)
    ...
    def _ensure_attachment_inputs(self, timeout=3000):
        ...
        try:
            ...
            log.debug("Optional step in _ensure_attachment_inputs() did not apply: %s", _ignored)
        ...
        if enabled:
            try:
                ...
                try:
                    ...
                    log.debug("Optional step in _click_send() did not apply: %s", _ignored)
        # Fallback: generic send icon (older/other layouts).
        return self.click_first_visible([CommonChatLocators.SEND_MESSAGE_ICON], "send message icon", timeout=timeout)
    ...
    def _send_attachment_until_visible(self, result_selectors, attempts=3):
        ...
        for _ in range(attempts):
            ...
            result = self.first_visible(result_selectors, timeout=12000)
            if result:
                return result
        return result
