...
from pages.common_pages.common_create_meeting_page import CommonCreateMeetingPage
...
def step_navigate_to_batch_details_and_upcoming(context):
    ...
    if not opened:
        log.warning("Batch details screen is not available (no openable batch); "
...
def _meeting_card_unavailable(context):
    if getattr(context, "meeting_card_unavailable", False):
        log.warning("Skipping create meeting step — meeting card did not surface for this batch.")
        return True
    return False
