...
from pages.common_pages.common_batch_members_page import CommonBatchMembersPage
...
def _batch_invite_unavailable(context):
    if getattr(context, "batchmembers_invite_unavailable", False):
        log.warning("Skipping batch members step — invite panel/batch code unavailable for this batch.")
        return True
    return False
...
def step_download_template_upload_and_invite(context):
    if _batch_invite_unavailable(context):
        return
    upload_file_path = require_test_data_file("WO-Student-bulk.xlsx")
    page = CommonBatchMembersPage(context.page)
