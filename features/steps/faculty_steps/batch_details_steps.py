...
def step_validate_toast_and_edit_faculty(context):
    if getattr(context, "faculty_add_skipped", False):
        log.warning("Skipping edit faculty validation — no faculty was added.")
        return
    page = BatchDetailsPage(context.page)
...
def step_delete_second_faculty(context):
    if getattr(context, "faculty_add_skipped", False):
        log.warning("Skipping delete faculty — no faculty was added.")
        return
    page = BatchDetailsPage(context.page)
...
def step_validate_faculty_delete_toast(context):
    if getattr(context, "faculty_add_skipped", False):
        log.warning("Skipping faculty delete toast validation — no faculty was added.")
        return
    page = BatchDetailsPage(context.page)
