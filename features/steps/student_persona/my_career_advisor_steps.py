...
def _onboarding_already_done(context):
    """True when this account has already completed Passions/Aptitudes.

    That onboarding is a one-time product flow - once finished, the Career
    Advisor card routes straight to matched-roles and the onboarding screens
    can never be rendered again, so the steps that drive them are skipped.
    """
    if getattr(context, "career_advisor_onboarded", False):
        log.info("Career Advisor onboarding is already complete for this account "
                 "- skipping the onboarding-only step.")
        return True
    return False
...
def click_my_career_advisor_card(context):
    ...
    # The page object reports whether this account already finished the
    # Passions/Aptitudes onboarding (the card then routes straight to
    # matched-roles). That flag drives the onboarding-only steps below.
    context.career_advisor_onboarded = page.navigate_to_my_career_advisor()
...
def validate_passion_header_and_click_review(context):
    if _onboarding_already_done(context):
        return
    page = MyCareerAdvisorPage(context.page)
